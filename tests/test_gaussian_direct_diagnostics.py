...
def test_candidate_normalization_and_high_surprise_use_their_own_bounds():
    ...
    for number, model in enumerate(direct.ASSUMED_MODELS):
        ...
        for metric_number, metric in enumerate(direct.DIAGNOSTICS):
            low, median, high = -4 + number, 2 + number, 6 + 2 * number + metric_number
            references[model][metric] = {
                "dm_low": low, "median": median, "dm_high": high, "summary_dim": 12,
            }
            distances[model][metric] = np.array(
                [low, median, high, high + 1] if number == 0
                else [high + 2, low - 1, high + 1, high + 2],
                dtype=float,
            )

    frame = direct.diagnostic_frame(index, distances, references, "cross_entropy")

    assert len(frame) == 4 * len(direct.ASSUMED_MODELS) * len(direct.DIAGNOSTICS)
    ...
    for (metric, model), panel in frame.groupby(["diagnostic", "assumed_model"]):
        ...
        np.testing.assert_allclose(
            panel["rho"],
            (distances[model][metric] - ref["median"]) / (ref["dm_high"] - ref["median"]),
        )
        np.testing.assert_allclose(
            panel["rho_low"],
            (ref["dm_low"] - ref["median"]) / (ref["dm_high"] - ref["median"]),
        )
        ...
        if model == "m1":
            ...
            assert panel["rho"].iloc[2] == 1
        else:
            assert panel["distance_regime"].iloc[1] == "interpolation"

    shifted = {
        model: {metric: values + 100 for metric, values in by_metric.items()}
        for model, by_metric in distances.items()
    }
    other = direct.diagnostic_frame(index, shifted, references, "logistic")
...
def isolated_direct_pipeline(tmp_path, monkeypatch):
    ...
    for source in direct.SOURCE_MODELS:
        items = [{
            ...
            "source_model": source,
            "id": 0,
            **{f"gold_log_marginal_{m}": float(i) for i, m in enumerate(direct.ASSUMED_MODELS)},
            **{f"npe_log_marginal_{m}": float(i) / 2 for i, m in enumerate(direct.ASSUMED_MODELS)},
        }]
    ...
    class Approximator:
        ...
        def estimate(self, conditions):
            return {"model_probs": np.tile([0.1, 0.2, 0.3, 0.4], (len(conditions["x"]), 1))}
    ...
    def fit_reference_suite(approximator, simulator, metrics, **settings):
        ...
        fitted.append((approximator.checkpoint.name, simulator.spec.copy(), summary.copy()))
        return {
            metric: {
                "summary_dim": 2, "dm_low": -1.0, "median": 0.0, "dm_high": 3.0,
                "mu_hat": summary.mean(axis=0), "L_hat": np.eye(2),
    ...
    fake_sd = SimpleNamespace(
        ...
        summary_distance_from_summary=lambda summary, mean, chol, metric: np.linalg.norm(summary - mean, axis=1),
        mmd_reference_distance_from_summary=lambda summary, reference, bandwidth, kernel: np.linalg.norm(summary - reference.mean(axis=0), axis=1),
        signed_typicality_from_summary=lambda summary, flow, expected: -summary[:, 0] / flow,
    )
    ...
    monkeypatch.setitem(sys.modules, "benchmark.examples.gaussian.analysis.summry_diagnostic", fake_sd)
    ...
    def run(loss, checkpoint):
        return direct.ensure_direct_diagnostics(
            loss, checkpoint=checkpoint, results_dir=tmp_path / "results",
            dataset_dir=dataset_dir, n_fit=8, n_calibration=8,
            n_boot=0, density_epochs=1, n_density_validation=8,
            reference_root=tmp_path / "shared_reference",
        )

    return SimpleNamespace(
        run=run, fitted=fitted, loaded=loaded, summarized=summarized,
        root=tmp_path, dataset_dir=dataset_dir,
    )


def test_each_loss_fits_each_candidate_in_its_own_direct_summary(isolated_direct_pipeline):
    ...
    for factor, loss in enumerate(direct.LOSSES, start=1):
        ...
        assert [call[1] for call in calls] == [direct.MODEL_SPECS[m] for m in direct.ASSUMED_MODELS]
        cache = pipeline.root / "results" / f"direct_{loss}" / "diagnostics" / "observed_summaries.npz"
        with np.load(cache) as saved:
            np.testing.assert_allclose(saved["summary"], np.tile([2 * factor, 4 + factor], (12, 1)))
        # Fitting the M1 and M2 distributions must use independent conditional draws.
        assert not np.array_equal(calls[0][2], calls[1][2])
    assert len(pipeline.fitted) == 3 * 4
...
def test_cache_tracks_checkpoint_content_and_output_integrity(isolated_direct_pipeline):
    ...
    for relative_path in ["diagnostics/diagnostic_frame.csv", "pmp_comparison.csv"]:
        ...
        assert len(pipeline.fitted) == 4  # Valid reference fits are still reusable.
