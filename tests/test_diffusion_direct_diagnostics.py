...
def test_pooled_rho_has_one_score_per_dataset_and_diagnostic():
    index = pd.DataFrame({"dataset": ["empirical"] * 4, "id": ["p0", "p1", "p2", "p3"]})
    references = {
        metric: {"low": -4.0, "high": 8.0, "summary_dim": 12}
...
def isolated_pipeline(tmp_path, monkeypatch):
    ...
    for relative in (
        "results/summary_diagnostic.py",
        "results/results.py",
        "results/observed_datasets.py",
        "results/shared_reference_fitting.py",
        "simulators/rdm_4.py",
        "dataset/dataset.py",
    ):
        path = base / relative
    ...
    class Approximator:
        ...
        def estimate(self, conditions):
            ...
            return {
                "model_probs": np.tile(
                    values / values.sum(), (len(conditions["rt"]), 1)
                )
            }
    ...
    class Simulator:
        ...
        def sample(self, count):
            ...
            y = np.tile(
                np.array([[[value, 0], [value + 2, 1]]], dtype=np.float32),
                (count, 1, 1),
            )
    ...
    def fit_reference_suite(approximator, simulator, metrics, **settings):
        ...
        fitted.append((approximator.path.name, list(simulator.consumed_splits), summary.copy()))
        return {
            metric: {
                "summary_dim": 2,
                "low": -1.0,
    ...
    fake_sd = SimpleNamespace(
        ...
        summary_distance_from_summary=lambda s, mean, chol, metric: np.linalg.norm(
            s - mean, axis=1
        ),
        mmd_reference_distance_from_summary=lambda s, ref, bandwidth, kernel: (
            np.linalg.norm(s - ref.mean(axis=0), axis=1)
        ),
    ...
    def shared_manifest(root=None, **settings):
        return {
            "generation": settings,
            "bank_directory": str(base / "reference_datasets"),
            "files": {
                m: {
                    part: {"array_sha256": f"{m}/{part}/{settings}"}
                    for part in ("fit", "calibration", "validation")
                }
                for m in direct.MODELS
            },
        }

    monkeypatch.setattr(
        helpers["shared_reference_data"],
        "ensure_shared_reference_data",
        shared_manifest,
    )
    def load_partition(manifest, model, part, *, bank=None):
        count = manifest["generation"][{"fit":"n_fit", "calibration":"n_calibration", "validation":"n_density_validation"}[part]]
        value = direct.MODELS.index(model) + 1
        return np.tile(np.array([[[value, 0], [value + 2, 1]]], dtype=np.float32), (count, 1, 1))

    monkeypatch.setattr(helpers["shared_reference_data"], "load_reference_partition", load_partition)
    monkeypatch.setattr(
        helpers["shared_reference_fitting"],
        "SharedReferenceSimulator",
        lambda manifest, model: Simulator(model),
    )
    monkeypatch.setitem(
        sys.modules,
        "benchmark.examples.diffusion.simulators.rdm_4",
        SimpleNamespace(RDM=Simulator),
    )
    ...
    monkeypatch.setitem(
        ...
        SimpleNamespace(
            ...
            utils=SimpleNamespace(
                set_random_seed=seeded.append, disable_interactive_logging=lambda: None
            ),
        ),
    )
...
def test_observations_follow_csv_dataset_id_order_and_reject_duplicates(
    isolated_pipeline,
):
    ...
    duplicate = pd.concat(
        [pipeline.indirect, pipeline.indirect.iloc[:1]], ignore_index=True
    )
...
def test_every_loss_scores_one_pooled_reference_in_its_own_summary(
    isolated_pipeline,
):
    pipeline = isolated_pipeline
    for factor, loss in enumerate(direct.LOSSES, start=1):
        ...
        assert set(calls[0][2][:, 0]) == {2 * factor, 3 * factor, 4 * factor, 5 * factor}
        ...
        density = frame.query("diagnostic == 'density'")
        np.testing.assert_allclose(density["distance"], expected_means)
        comparison = pd.read_csv(output / "pmp_comparison.csv")
        np.testing.assert_allclose(comparison["p_direct_m0"], factor / (factor + 9))
        assert len(frame) == len(pipeline.indirect) * 4
        ...
        # One observed summary batch plus one pooled reference-fit batch.
        assert (
            len([call for call in pipeline.summarized if call[0] == checkpoint.name])
            == 2
        )
    assert not (pipeline.base / "calibration_reference_100").exists()
    assert len(pipeline.fitted) == 3
    assert pipeline.seeded == []
...
def test_cache_tracks_content_and_repairs_outputs_without_refitting(isolated_pipeline):
    ...
    for relative in (
        "pmp_comparison.csv",
        "diagnostics/diagnostic_frame.csv",
...
def test_changed_observations_and_indirect_comparison_rescore_but_settings_refit(
    isolated_pipeline,
):
    pipeline = isolated_pipeline
    checkpoint = pipeline.root / "direct_cross_entropy.keras"
    ...
    pipeline.run("cross_entropy", checkpoint)
    assert len(pipeline.loaded) == 2 and len(pipeline.fitted) == 1
    pipeline.indirect["pmp_m0"] = 0.4
    pipeline.indirect["pmp_m1"] = 0.1
    pipeline.indirect.to_csv(pipeline.indirect_path, index=False)
    pipeline.run("cross_entropy", checkpoint)
    assert len(pipeline.loaded) == 3 and len(pipeline.fitted) == 1
