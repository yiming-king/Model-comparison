...
def shared(tmp_path, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location(
        package_name + ".shared_reference_data", source
    )
    ...
    class Simulator:
        ...
        def sample(self, count):
            ...
            return {
                "rt": np.random.normal(size=(count, 2)),
    ...
    monkeypatch.setitem(
        sys.modules,
        "benchmark.examples.diffusion.dataset",
        SimpleNamespace(wagenmakers=SimpleNamespace(conditions=np.array([0, 1]))),
    )

    def ensure(**kwargs):
        return module.ensure_shared_reference_data(
            root=tmp_path / "bank",
            n_fit=4,
            n_calibration=5,
            n_density_validation=6,
            **kwargs,
        )
...
def test_reference_bank_is_persisted_and_identical_for_every_network(shared):
    ...
    for model in module.MODELS:
        ...
        for count, part in zip((4, 5, 6), module.PARTITIONS):
            ...
            assert (
                module.array_sha256(saved)
                == manifest["files"][model][part]["array_sha256"]
            )
...
def test_validation_random_stream_is_independent_of_fit_partition_length(shared):
    manifest = shared.ensure()
    different = shared.module.ensure_shared_reference_data(
        root=shared.root / "bank", n_fit=7, n_calibration=5, n_density_validation=6
    )
    for model in shared.module.MODELS:
        ...
        assert (
            manifest["files"][model]["calibration"]["array_sha256"]
            != different["files"][model]["calibration"]["array_sha256"]
        )
...
def test_indirect_diagnostic_cache_tracks_reference_and_observation_content(shared):
    ...
    names = {
        ...
        "compute_all_observed_summary_diagnostics",
    }
    functions = [
        node
    ...
    module.write_json(
        metadata,
        {
            "identity": {"shared_reference_data": "raw bank A"},
    ...
    namespace = {
        ...
        "np": np,
        "pd": pd,
        "REFERENCE_METRICS": ("l2",),
        "OBSERVED_DATASETS": ("empirical",),
        "NPE_RESULT_DIR": root,
        ...
        "array_sha256": module.array_sha256,
    ...
    run = lambda: namespace["ensure_observed_summary_diagnostics"](
        config, results, {}, {}
    )
    first = run()["l2"]
    ...
    module.write_json(
        metadata,
        {
            "identity": {"shared_reference_data": "raw bank B"},
            "sha256": module.file_sha256(reference),
        },
    )
    run()
...
def test_indirect_metric_subset_preserves_the_complete_verified_reference_suite(shared):
    ...
    function = next(
        ...
        and node.name == "load_or_fit_reference_suite"
    )
    ...
    def verified_candidate(approximator, **kwargs):
        ...
        return {
            metric: {"from_verified_shared_bank": True} for metric in kwargs["metrics"]
        }
    ...
    namespace = {
        ...
        "file_sha256": module.file_sha256,
        "reference_data_identity": module.reference_data_identity,
