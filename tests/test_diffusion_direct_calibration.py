"""Direct diffusion PMP calibration reuses the indirect benchmark and gold PMP."""
...
from benchmark.examples.direct_pmp_calibration import calibrate_direct_pmp_signed
            ...
            for dataset_id, errors in enumerate(
                ...
                for candidate, error in zip(direct.MODELS, errors):
                    rows.append(
                        {
                            "loss_function": loss,
                            "generating_model": generator,
                            "dataset_id": f"s{dataset_id:03d}",
                            "candidate_model": candidate,
                            "gold_pmp": 0.25,
                            "direct_pmp": 0.25 + error / (loss_index + 1),
                        }
                    )
...
def test_thresholds_use_signed_errors_per_candidate(coverage):
    metrics = _metrics()
    errors, result = calibrate_direct_pmp_signed(metrics, direct.MODELS, "gold_pmp", coverage)
    pd.testing.assert_frame_equal(result, direct.calculate_direct_pmp_thresholds(metrics, coverage))
    assert len(result) == 3 * 4
    assert result.n_values.eq(12).all()
    assert len(errors) == 3 * 4 * 3 * 4
    assert set(result.metric) == {"signed_pmp_error"}
    for row in result.itertuples():
        sample = errors.loc[
            errors.loss_function.eq(row.loss_function)
            & errors.candidate_model.eq(row.candidate_model), "signed_pmp_error"
        ]
        np.testing.assert_allclose(
            [row.lower_threshold, row.threshold],
            np.quantile(sample, [(1 - coverage) / 2, 1 - (1 - coverage) / 2]),
        )


def test_missing_candidates_and_generators_cannot_enter_thresholds():
    metrics = _metrics()
    with pytest.raises(ValueError, match="all four PMP components"):
        direct.calculate_direct_pmp_thresholds(metrics.iloc[1:])
    missing_generator = _metrics().loc[
        lambda x: ~(x.loss_function.eq("logistic") & x.generating_model.eq("m3"))
    ]
...
def benchmark(tmp_path):
    ...
    for generator_index, generator in enumerate(direct.MODELS):
        ...
        pd.DataFrame({"id": ids, "dataset_seed": [31, 32, 33]}).to_csv(
            directory / "true_parameters.csv", index=False
        )
        manifests.append(
            pd.DataFrame(
                {"generating_model": generator, "id": ids, "dataset_seed": [31, 32, 33]}
            )
        )
        for number, dataset_id in enumerate(ids):
            record = {
                "N": 2,
                "rt": [0.4 + number / 10, -0.5 - generator_index / 10],
                "condition": [1, 0],
            }
    ...
    pd.concat(manifests, ignore_index=True).to_csv(
        root / "dataset_manifest.csv", index=False
    )
    gold_rows = [
        {"generating_model": generator, "dataset_id": dataset_id,
         "candidate_model": candidate, "gold_pmp": value}
        for generator in direct.MODELS for dataset_id in ids
        for candidate, value in zip(direct.MODELS, [0.1, 0.2, 0.3, 0.4])
    ]
    pd.DataFrame(gold_rows).to_csv(root / "per_dataset_metrics.csv", index=False)
...
def test_loader_preserves_saved_rt_condition_ids_and_mcmc_alignment(benchmark):
    ...
    assert index.generating_model.tolist() == [
        m for m in direct.MODELS for _ in range(3)
    ]
    assert index.all_candidate_models_converged.tolist() == [True, True, True] * 4
...
def _checkpoint(path, probability):
    config = {
        ...
        "build_config": {
            "input_shape": {
                "summary_variables": [None, 2, 2],
                "inference_variables": [None, 4],
            }
        },
        "config": {
            ...
            "inference_network": {
                "config": {
                    "scoring_rules": {
                        "scoring_rule": {"class_name": "CrossEntropyScore"}
                    }
                }
            },
        },
...
def test_cached_calibration_uses_diffusion_adapter_and_tracks_checkpoint_and_gold(
    ...
    def load_model(path, compile=False):
        ...
        def estimate(conditions):
            ...
            evaluated.append(
                np.stack([conditions["rt"], conditions["conditions"]], axis=-1)
            )
            return {
                "model_probs": np.tile(
                    [probability, 0.2, 0.3, 0.5 - probability],
                    (len(conditions["rt"]), 1),
                )
            }
    ...
    monkeypatch.setitem(
        sys.modules,
        "keras",
        SimpleNamespace(saving=SimpleNamespace(load_model=load_model)),
    )
    kwargs = {
        "checkpoint": checkpoint,
        "benchmark_dir": root,
        "gold_metrics_path": root / "per_dataset_metrics.csv",
    ...
    for candidate_index, model in enumerate(direct.MODELS):
        ...
        np.testing.assert_allclose(
            frame.gold_pmp, [0.1, 0.2, 0.3, 0.4][candidate_index]
        )
    np.testing.assert_allclose(
        metrics.signed_pmp_error, metrics.direct_pmp - metrics.gold_pmp
    )
    assert not (output / "per_dataset_l1_errors.csv").exists()
    for threshold in first.itertuples():
        sample = metrics.loc[metrics.candidate_model.eq(threshold.candidate_model), "signed_pmp_error"]
        np.testing.assert_allclose(
            [threshold.lower_threshold, threshold.threshold], sample.quantile([0.05, 0.95])
        )
    ...
    np.testing.assert_allclose(
        updated.loc[updated.candidate_model.eq("m0"), "direct_pmp"], 0.2
    )
...
def test_missing_direct_checkpoint_is_explicit_and_never_creates_placeholder(tmp_path):
    with pytest.raises(FileNotFoundError, match="checkpoint is missing"):
        direct.ensure_direct_pmp_thresholds(
            "exponential",
            checkpoint=tmp_path / "missing.keras",
            results_dir=tmp_path / "results",
        )
...
def test_cross_entropy_checkpoint_cannot_stand_in_for_missing_other_loss(tmp_path):
    ...
    with pytest.raises(ValueError, match="scoring rule does not match"):
        direct.ensure_direct_pmp_thresholds(
            "logistic", checkpoint=path, results_dir=tmp_path / "results"
        )
    assert not (tmp_path / "results").exists()
