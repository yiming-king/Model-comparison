"""Check dataset-level direct PMP calibration on the saved Gaussian benchmark."""
...
from benchmark.examples.direct_pmp_calibration import calibrate_direct_pmp_signed, load_indirect_gold
                    ...
                    rows.append(
                        {
                            ...
                            "analytical_pmp": 0.25,
                            "direct_pmp": 0.25 + error / (1 + loss_index + model_index),
                        }
                    )
...
def test_direct_thresholds_use_signed_errors_per_candidate(coverage):
    metrics = _component_metrics()
    errors, actual = calibrate_direct_pmp_signed(
        metrics, direct.ASSUMED_MODELS, "analytical_pmp", coverage
    )
    pd.testing.assert_frame_equal(
        actual, direct.calculate_direct_pmp_thresholds(metrics, coverage)
    )
    assert len(actual) == 3 * 4
    assert len(errors) == 3 * 4 * 2 * 4
...
def saved_benchmark(tmp_path):
    ...
    for item in index.itertuples(index=False):
        delta = item.dataset_id / 1000
        for candidate, value in zip(direct.ASSUMED_MODELS, [0.1 + delta, 0.2, 0.3, 0.4 - delta]):
            gold_rows.append({"generating_model": item.generating_model,
                              "dataset_id": item.dataset_id,
                              "candidate_model": candidate, "analytical_pmp": value})
    pd.DataFrame(gold_rows).to_csv(root / "per_dataset_metrics.csv", index=False)
    return root, np.concatenate(arrays), index


def test_benchmark_loader_preserves_saved_values_manifest_ids_and_order(
    saved_benchmark,
):
...
def test_indirect_gold_is_reused_in_saved_benchmark_order(saved_benchmark):
    root, _, index = saved_benchmark
    gold = load_indirect_gold(
        root / "per_dataset_metrics.csv", index, direct.ASSUMED_MODELS, "analytical_pmp"
    )
    assert gold.iloc[0].dataset_id == 91
    assert gold.iloc[4].dataset_id == 4
    assert gold.iloc[0].analytical_pmp == pytest.approx(0.191)
...
def test_calibration_cache_evaluates_saved_data_and_invalidates_checkpoint_content(
    ...
    for model, frame in metrics.groupby("candidate_model", sort=False):
        pd.testing.assert_frame_equal(
            frame[["generating_model", "dataset_id"]].reset_index(drop=True),
            expected_index,
        )
    np.testing.assert_allclose(
        metrics.signed_pmp_error, metrics.direct_pmp - metrics.analytical_pmp
    )
    ...
    np.testing.assert_allclose(
        updated_metrics.loc[updated_metrics.candidate_model.eq("m1"), "direct_pmp"],
        0.2,
    )
