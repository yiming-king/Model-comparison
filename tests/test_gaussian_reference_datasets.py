...
def _bank(tmp_path):
    return shared.ensure_reference_datasets(
        3, 4, n_fit=8, n_calibration=9, n_density_validation=10,
        seed=2025, root=tmp_path / "raw_reference",
    )


def test_persisted_reference_batches_match_historical_direct_rng_exactly(tmp_path):
    bank = _bank(tmp_path)
    files_before = {path: (path.stat().st_mtime_ns, path.read_bytes()) for path in bank.root.rglob("*.npy")}
    ...
    for index, model in enumerate(shared.ASSUMED_MODELS):
        historical = _GaussianSimulator(shared.MODEL_SPECS[model], 4, 3, 2025 + 10_000 * index)
        for split, count in zip(shared.SPLITS, (8, 9, 10)):
            np.testing.assert_array_equal(bank.load(model, split), historical.sample(count)["x"])
    assert len(bank.identity["file_sha256"]) == 12


def test_shared_reference_replays_whole_splits_without_resampling_or_wraparound(tmp_path):
    bank = _bank(tmp_path)
...
def test_changed_shared_raw_content_or_configuration_cannot_be_silently_reused(tmp_path):
    bank = _bank(tmp_path)
...
def reference_suite(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sd, "typicality_log_density", lambda flow, values: -np.square(values).sum(axis=1))
    monkeypatch.setattr(sd, "density_flow_validation", validation)
    bank, approximator = _bank(tmp_path), Approximator()
    settings = {
        "n_fit": 8, "n_calibration": 9, "n_density_validation": 10,
        "alpha": 0.1, "n_boot": 0, "seed": 2025,
        "density_epochs": 1, "density_batch_size": 8,
    }
...
def test_legacy_reuse_requires_actual_embeddings_from_each_shared_batch(reference_suite, changed_split):
    bank, approximator, refs, settings, _, _ = reference_suite
    report = shared.validate_legacy_direct_reference(refs, approximator, bank, "m1", settings)
    ...
    updated_metadata["file_sha256"][f"m1/{changed_split}.npy"] = shared.file_sha256(path)
    ...
    with pytest.raises(ValueError, match="does not match shared data"):
        shared.validate_legacy_direct_reference(refs, approximator, changed_bank, "m1", settings)


def test_verified_legacy_direct_flow_is_reused_and_evidence_is_recorded(reference_suite, tmp_path):
    ...
    legacy = {
        "identity": {
            "schema_version": 1, "summary_owner": "direct_cross_entropy",
            "checkpoint_sha256": shared.file_sha256(checkpoint), "settings": settings,
        },
        "sha256": shared.file_sha256(path),
    }
    path.with_suffix(".json").write_text(json.dumps(legacy))
    kwargs = dict(checkpoint=checkpoint, settings=settings, metrics=tuple(refs), allow_legacy_direct=True)
    ...
    assert "full_fit_embedding_matrix" in metadata["provenance"]["maximum_absolute_differences"]
...
def test_unproven_legacy_indirect_reference_is_refitted(reference_suite, tmp_path):
    ...
    path = tmp_path / "reference_m1.pkl"
    sd.save_reference_suites(refs, path)  # Historical pickle has no shared-input provenance.
    shared.load_or_fit_model_reference(
        path, approximator, bank, "m1", checkpoint=checkpoint,
        settings=settings, metrics=tuple(refs),
    )
    assert len(fit_calls) == 2
    metadata = json.loads(path.with_suffix(".json").read_text())
...
def test_indirect_metric_subset_preserves_other_verified_bundle_metrics(reference_suite, tmp_path):
    ...
    kwargs = dict(
        overwrite=False, reference_kwargs=settings,
        shared_reference_data=bank, checkpoint_paths=checkpoints,
    )
