...
def misspec_label(assumed: str, source: str) -> str:
    ...
    prior_diff = not (
        np.isclose(a["mu_prior_mean"], s["mu_prior_mean"])
        and np.isclose(a["mu_prior_std"], s["mu_prior_std"])
    )
    likelihood_diff = not np.isclose(a["likelihood_std"], s["likelihood_std"])
    return (
        "J"
        if prior_diff and likelihood_diff
        else "P"
        if prior_diff
        else "L"
        if likelihood_diff
...
MISSPEC_LABELS = {
    a: {s: misspec_label(a, s) for s in SOURCE_MODELS} for a in ASSUMED_MODELS
}
...
def stack_obs(data: list[dict] | dict | np.ndarray, obs_key: str = "x") -> np.ndarray:
    ...
    return np.stack(
        [np.asarray(item[obs_key], dtype=np.float32) for item in data], axis=0
    )


def summary_outputs(
    approximator, x_batch: np.ndarray, obs_key: str = "x"
) -> np.ndarray:
...
def fit_reference(
    ...
    stats = bootstrap_reference_stats(
        calibration_distances, alpha=alpha, n_boot=n_boot, seed=bootstrap_seed
    )
    reference.update(
        {
            "median": stats["median"],
...
def bootstrap_reference_stats(
    ...
    return {
        ...
        "dm_low": float(qs[1].mean()),
        "dm_high": float(qs[2].mean()),
    }
...
def fit_reference_suite(
    ...
    if "density" in metrics:
        ...
        stats = {
            ...
            "dm_low": float(np.percentile(signed_surprise_distance, 100 * alpha / 2)),
            "dm_high": float(
        ...
        references["density"] = {
            ...
            "typicality_tau": float(max(abs(stats["dm_low"]), abs(stats["dm_high"]))),
            "alpha": alpha,
...
def compute_logml_and_posteriors(
    ...
    for assumed in assumed_models:
        ...
        for source in sources:
            ...
            datasets[source] = calculation.npe_estimation_use_gold_posterior(
                datasets[source]
            )
    return datasets
...
def collect_posterior_distance_frame(
    ...
    for source in sources:
        for item in datasets[source]:
            for assumed in assumed_models:
                gold = np.asarray(
                    item[f"gold_post_samples_{assumed}"], dtype=np.float32
                )
                npe = np.asarray(item[f"npe_post_samples_{assumed}"], dtype=np.float32)
                sample_count = min(n_samples, len(gold), len(npe))
                if sample_count <= 1:
                    raise ValueError(
                        "MMD requires at least two posterior samples from each distribution"
                    )
                ...
                rows.append(
                    {
                        ...
                        "posterior_mean_rmse": float(
                            np.sqrt(
                                np.mean((npe.mean(axis=0) - gold.mean(axis=0)) ** 2)
                            )
                        ),
    ...
    for source in sources:
        for item in datasets[source]:
            ...
            row = {
                ...
                "at_least_one_not_extrapolative": bool(
                    item["at_least_one_not_extrapolative"]
                ),
                "not_extrapolative_count": sum(
                    v != "extrapolation" for v in item["summary_regimes"].values()
                ),
                "ambiguity_score": float(item["ambiguity_score"]),
                "ambiguity_score_true": float(
                    item.get(
                        "ambiguity_score_true",
                        1.0 / (abs(item["d_second"] - item["d_min"]) + 1e-8),
                    )
                ),
            ...
            row["extrapolation_class"] = _extrapolation_class(
                row["not_extrapolative_count"]
            )
            row["nearest_two_extrapolation_class"] = _nearest_two_extrapolation_class(
                item["closest_summary_models"], item["summary_regimes"]
            )
            for j, assumed in enumerate(assumed_models):
                ...
                row[f"dm_median_{assumed}"] = float(
                    item["summary_ci"][assumed]["median"]
                )
                ...
                row[f"signed_pmp_error_direct_{assumed}"] = (
                    float(direct[j] - gold[j]) if direct_ok else np.nan
                )
            rows.append(row)
    return pd.DataFrame(rows)
...
def summarize_frames(
    logml_df: pd.DataFrame, pmp_df: pd.DataFrame
) -> tuple[pd.DataFrame, pd.DataFrame]:
    logml_summary = (
        logml_df.groupby(
            ["assumed_model", "source_model", "distance_regime"], sort=False
        )
        .agg(
            n=("id", "count"),
            median_d_M=("d_M", "median"),
            median_signed_logml_error=("signed_logml_error", "median"),
        )
        .reset_index()
    )
    pmp_summary = (
        ...
        .agg(
            n=("id", "count"),
            median_d_min=("d_min", "median"),
            median_A=("ambiguity_score", "median"),
            median_pmp_l1_error_npe=("pmp_l1_error_npe", "median"),
        )
        .reset_index()
    )
...
def _add_first_large_error(
    ax, data: pd.DataFrame, x_col: str, y_col: str, error_bound: float | None
) -> None:
...
def _error_subset_data(data: pd.DataFrame, subset: str | None) -> pd.DataFrame:
    ...
    raise ValueError(
        "error_subset must be None, 'all_extrapolative', or 'not_all_extrapolative'"
    )
...
def _normalize_distance(x, high):
    upper = np.asarray(high, dtype=float)
    if np.any(upper <= 0) or not np.isfinite(upper).all():
        raise ValueError("Diagnostic upper threshold must be positive and finite")
    return np.asarray(x, dtype=float) / upper
...
def _diagnostic_xlabel(distance_metric: str) -> str:
    ...
    if metric not in DIAGNOSTIC_XLABELS:
        raise ValueError(f"distance_metric must be one of {tuple(DIAGNOSTIC_XLABELS)}")
...
def _nearest_two_extrapolation_class(
    models: list[str] | tuple[str, ...] | str, regimes: dict[str, str]
) -> str:
...
def _with_extrapolation_class(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "extrapolation_class" not in out.columns:
        ...
        out["extrapolation_class"] = out["not_extrapolative_count"].map(
            _extrapolation_class
        )
    if "nearest_two_extrapolation_class" not in out.columns:
        ...
        out["nearest_two_extrapolation_class"] = [
            ...
            for idx, row in zip(
                order,
                out[regimes]
                .rename(columns=lambda x: x.replace("regime_", ""))
                .to_dict("records"),
                strict=False,
            )
        ]
...
def plot_logml_error_vs_distance(
    ...
    if color_by not in {"source", "gold_logml", "npe_logml", "signed_logml_error"}:
        raise ValueError(
            "color_by must be 'source', 'gold_logml', 'npe_logml', or 'signed_logml_error'"
        )
    ...
    fig, axes = plt.subplots(
        1, len(assumed_models), figsize=(5.1 * len(assumed_models), 5.2), sharey=True
    )
    ...
    for ax, assumed in zip(axes, assumed_models, strict=False):
        ...
        calibrated_error = not normalized_error and calibrated_columns.issubset(
            sub.columns
        )
        ...
        if normalized_error:
            median = _error_median(sub, "signed_logml_error", assumed, error_median)
            ...
            error_low = float(
                sub["signed_logml_error_lower_threshold"].median() / np.log(10.0)
            )
            error_high = float(
                sub["signed_logml_error_threshold"].median() / np.log(10.0)
            )
        if use_rho:
            rho = _normalize_distance(sub["d_M"], sub["dm_high"])
            ...
            low = (
                float(_safe_log(sub["dm_low"].iloc[0]))
                if use_log_distance
                else float(sub["dm_low"].iloc[0])
            )
            high = (
                float(_safe_log(sub["dm_high"].iloc[0]))
                if use_log_distance
                else float(sub["dm_high"].iloc[0])
            )
    ...
    if output_dir is not None:
        ...
        fig.savefig(
            Path(output_dir) / (filename or f"logml_error_vs_{suffix}_by_assumed.png"),
            dpi=200,
            bbox_inches="tight",
        )
    return fig, axes
...
def _pmp_long_frame(pmp_df: pd.DataFrame, estimate: str = "npe") -> pd.DataFrame:
    ...
    if "ambiguity_score_true" not in pmp_df:
        pmp_df["ambiguity_score_true"] = 1.0 / (
            np.abs(pmp_df["d_second"] - pmp_df["d_min"]) + 1e-8
        )
    rows = []
    for model in ASSUMED_MODELS:
        cols = [
            ...
            "extrapolation_class",
            "nearest_two_extrapolation_class",
            "ambiguity_score",
            "ambiguity_score_true",
            "d_min",
            "d_second",
            ...
            f"p_npe_{model}",
            f"p_direct_{model}",
            f"signed_pmp_error_{estimate}_{model}",
        ]
        ...
        part["rho_M"] = _normalize_distance(part["d_M"], part["dm_high"])
        part["rho_low"] = _normalize_distance(part["dm_low"], part["dm_high"])
...
def _pmp_rmse_frame(pmp_df: pd.DataFrame, estimate: str = "npe") -> pd.DataFrame:
    ...
    if "ambiguity_score_true" not in out:
        out["ambiguity_score_true"] = 1.0 / (
            np.abs(out["d_second"] - out["d_min"]) + 1e-8
        )
    error_cols = [f"signed_pmp_error_{estimate}_{m}" for m in ASSUMED_MODELS]
    out["pmp_rmse"] = np.sqrt(
        np.mean(np.square(out[error_cols].to_numpy(float)), axis=1)
    )
...
def _nearest_distance_region(
    data: pd.DataFrame,
) -> tuple[
    float, float, float
]:  # return typical low/high distance values and max x for plotting based on nearest assumed model
    d_cols = [f"d_{m}" for m in ASSUMED_MODELS]
    nearest = (
        data[d_cols].to_numpy().argmin(axis=1)
    )  # return indices of nearest assumed model for each row
    lows = [data[f"dm_low_{ASSUMED_MODELS[j]}"].iloc[i] for i, j in enumerate(nearest)]
    highs = [
        data[f"dm_high_{ASSUMED_MODELS[j]}"].iloc[i] for i, j in enumerate(nearest)
    ]
    ...
    return (
        ...
        x_max,
    )  # median low/high distance values across rows based on nearest assumed model, and max x for plotting
...
def _source_handles() -> list[Line2D]:
    return [
        Line2D(
            ...
            marker="o",
            color="none",
            markerfacecolor=SOURCE_COLORS[i],
            markeredgecolor="black",
            markersize=7,
            label=source.upper(),
        )
...
def _marker_legend(
    fig, marker_map: dict[object, tuple[str, int]], y: float = -0.07
) -> None:
    fig.legend(
...
def _pmp_plot_data(
    pmp_df: pd.DataFrame, y: str, estimate: str
...
def _pmp_x_column(
    ...
    if x in {"log_distance", "logdistance"}:
        if model is None:
            raise ValueError(
                "x='log_distance' is only available for model-wise PMP plots"
            )
        j = _pmp_model_index(model)
    ...
    raise ValueError(
        "x must be 'distance', 'log_distance', 'rho', 'log_rho', 'A', 'A_true', 'd_min', or 'log_d_min'"
    )
...
def _scatter_pmp(ax, data: pd.DataFrame, x_col: str, y_col: str, group_by: str):
    if group_by == "source_model":
        ...
        ax.scatter(
            data[x_col],
            data[y_col],
            c=data["source_model"].map(color_map),
            s=36,
            alpha=0.75,
            edgecolors="black",
            linewidths=0.4,
    ...
    if group_by == "global_extrapolation_source":
        ...
        for group_value, (marker, size) in marker_map.items():
            ...
            ax.scatter(
                group[x_col],
                group[y_col],
                c=group["source_model"].map(color_map),
                s=size,
                alpha=0.75,
                marker=marker,
                edgecolors="black",
    ...
    group_col = (
        "at_least_one_not_extrapolative"
        if group_by == "global_extrapolation"
        else "nearest_two_extrapolation_class"
    )
    marker_map = {
...
def plot_pmp_diagnostic(
    ...
    for ax, (
        ...
        y_bounds = (
            (-1.0, 1.0)
            if normalized_error
            else (threshold_low, threshold_high)
            if calibrated_error
            else None
        )
        if regions == "assumed":
...
def plot_pmp_estimates_vs_distance(
    ...
    for ax, model in zip(axes, ASSUMED_MODELS, strict=False):
        ...
        low = (
            float(_safe_log(data[f"dm_low_{model}"].iloc[0]))
            if use_log
            else float(data[f"dm_low_{model}"].iloc[0])
        )
        high = (
            float(_safe_log(data[f"dm_high_{model}"].iloc[0]))
            if use_log
            else float(data[f"dm_high_{model}"].iloc[0])
        )
        ...
        last = ax.scatter(
            ...
            alpha=0.78,
            marker="o",
            label="NPE" if y == "estimate" else "NPE - gold",
        )
        ax.scatter(
            x_values,
            direct_y,
            ...
            edgecolors="black",
            linewidths=0.5,
            label="NPMP" if y == "estimate" else "NPMP - gold",
        )
        ax.set_title(rf"$p(M_{model[-1]} \mid y)$")
        ax.set_xlim(x_min, x_max)
        ax.set_xlabel(
            rf"$\log d_{model[-1]}(y)$" if use_log else rf"$d_{model[-1]}(y)$"
        )
        _apply_axis_scales(
            ax,
            xscale=xscale,
    ...
    fig.suptitle(
        f"Observation datasets from {source_model.upper()}",
        y=0.96,
        fontsize=PLOT_FONT["suptitle"],
    )
    ...
    if output_dir is not None:
        ...
        fig.savefig(
            Path(output_dir) / f"{prefix}_vs_{suffix}_{source_model}.png",
            dpi=200,
            bbox_inches="tight",
        )
    return fig, axes
