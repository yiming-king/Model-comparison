...
class ReferenceSimulator:
    ...
    def sample(self, count: int) -> dict:
        ...
        if len(values) != count:
            raise ValueError(f"Shared reference {split} contains {len(values)} observations, requested {count}")
...
def ensure_reference_datasets(
    data_dim: int = 20, num_obs: int = 10, *, n_fit: int = 2000,
    n_calibration: int = 2000, n_density_validation: int = 2000,
    seed: int = 2025, root: str | Path | None = None,
) -> ReferenceDatasets:
    ...
    """
    counts = dict(zip(SPLITS, (n_fit, n_calibration, n_density_validation), strict=True))
    if data_dim < 1 or num_obs < 1 or min(counts.values()) < 2:
        raise ValueError("Reference dimensions must be positive and each split requires at least two datasets")
    configuration = {
        "generator": "gaussian_vectorized_sequential_batches_v1",
        "data_dim": int(data_dim), "num_obs": int(num_obs), "seed": int(seed),
        "counts": counts, "dtype": "float32",
        "model_specs": {model: MODEL_SPECS[model] for model in ASSUMED_MODELS},
    }
    folder = Path(root) if root is not None else (
        DEFAULT_ROOT / configuration_tag(data_dim, num_obs)
    ...
    if manifest.exists():
        ...
        if metadata.get("configuration") != configuration:
            raise ValueError(f"Existing shared reference configuration differs: {manifest}")
        bank = ReferenceDatasets(folder, metadata)
        for model in ASSUMED_MODELS:
            for split in SPLITS:
                values = bank.load(model, split)
                if values.shape != (counts[split], num_obs, data_dim) or values.dtype != np.float32 or not np.isfinite(values).all():
    ...
    for model_index, model in enumerate(ASSUMED_MODELS):
        ...
        for split, count in counts.items():
            mu = rng.normal(spec["mu_prior_mean"], spec["mu_prior_std"], size=(count, 1, data_dim))
            values = rng.normal(mu, spec["likelihood_std"], size=(count, num_obs, data_dim)).astype(np.float32)
            ...
            if path.exists():
                if not np.array_equal(np.load(path, allow_pickle=False), values):
                    raise ValueError(f"Unmanifested shared reference has different data: {path}")
            else:
    ...
    metadata = {"schema_version": 1, "configuration": configuration, "file_sha256": hashes}
    _write_json(manifest, metadata)
    return ReferenceDatasets(folder, metadata)


def validate_legacy_direct_reference(references, approximator, bank, model, settings) -> dict:
    ...
    summaries = {split: sd.summary_outputs(approximator, bank.load(model, split)) for split in SPLITS}
    checks = {}

    def check(name, actual, expected):
        actual, expected = np.asarray(actual), np.asarray(expected)
        if actual.shape != expected.shape or not np.allclose(actual, expected, rtol=2e-6, atol=2e-6):
            raise ValueError(f"Legacy reference does not match shared data: {model}/{name}")
    ...
    check("mmd_reference_kernel_mean", kernel_mean, references["mmd"]["reference_kernel_mean"])
    for metric in ("l2", "linf", "mmd"):
        reference = references[metric]
        if metric == "mmd":
            distances = sd.mmd_reference_distance_from_summary(calibration, fit, bandwidth, kernel_mean)
        else:
            check(f"{metric}_fit_mean", covariance.location_, reference["mu_hat"])
            check(f"{metric}_fit_cholesky", chol, reference["L_hat"])
            distances = sd.summary_distance_from_summary(calibration, covariance.location_, chol, metric=metric)
        statistics = sd.bootstrap_reference_stats(distances, alpha=settings["alpha"], n_boot=settings["n_boot"], seed=settings["seed"])
    ...
    quantiles = np.percentile(signed, [50, 100 * settings["alpha"] / 2, 100 * (1 - settings["alpha"] / 2)])
    ...
    validation = sd.density_flow_validation(
        density["flow"], summaries["validation"], num_flow_samples=settings["n_density_validation"], seed=settings["seed"],
    )
    ...
    return {
        ...
        "recomputed_embedding_sha256": {split: _array_sha256(values) for split, values in summaries.items()},
        "rtol": 2e-6, "atol": 2e-6,
    }


def load_or_fit_model_reference(
    path: Path, approximator, bank: ReferenceDatasets, model: str, *,
    checkpoint: Path, settings: dict, metrics: tuple[str, ...], overwrite: bool = False,
    ...
    identity = {
        "schema_version": 2, "candidate_model": model,
        "checkpoint": str(checkpoint.resolve()), "checkpoint_sha256": file_sha256(checkpoint),
        "shared_reference_datasets": bank.identity, "settings": settings,
        "metrics": sorted(metrics), "implementation_sha256": file_sha256(Path(__file__)),
        "diagnostic_implementation_sha256": file_sha256(Path(__file__).with_name("summry_diagnostic.py")),
        "checkpoint_loader_sha256": file_sha256(Path(__file__).parents[1] / "approximators" / "legacy_npe.py"),
    }
    ...
    compatible_identity = (
        ...
        == {k: v for k, v in identity.items() if k != "metrics"}
        and set(metrics).issubset(prior_identity.get("metrics", []))
    )
    ...
    if not overwrite and valid_file and allow_legacy_direct:
        ...
        expected_old_settings = {**settings, "seed": bank.metadata["configuration"]["seed"]}
        if (old.get("schema_version") == 1 and str(old.get("summary_owner", "")).startswith("direct_")
            ...
            try:
                migration = validate_legacy_direct_reference(candidate, approximator, bank, model, settings)
            except (KeyError, ValueError, TypeError) as error:
                print(f"Legacy {model} reference requires refitting: {error}", flush=True)
            else:
                migration["legacy_identity"] = old
                references = candidate
                print(f"Verified legacy {model} reference against shared raw datasets", flush=True)
    if migration is None:
        keras.utils.set_random_seed(settings["seed"])
        simulator = bank.simulator(model)
        references = sd.fit_reference_suite(approximator, simulator, metrics=metrics, **settings)
        expected_splits = list(SPLITS if "density" in metrics else SPLITS[:2])
        if simulator.consumed_splits != expected_splits:
            raise ValueError(f"Reference fit did not consume expected shared splits: {simulator.consumed_splits}")
        sd.save_reference_suites(references, path)
        provenance = {"method": "fitted using persisted shared raw datasets", "consumed_splits": simulator.consumed_splits}
    else:
        provenance = migration
    _write_json(metadata_path, {"identity": identity, "sha256": file_sha256(path), "provenance": provenance})
    return references
