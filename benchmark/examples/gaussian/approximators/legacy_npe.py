...
from bayesflow.networks.summary.transformers.attention.multihead_attention import MultiHeadAttention
...
class LegacyAttention(MultiHeadAttention):
    def __init__(self, **kwargs):
        ...
        self.input_projector = keras.layers.Dense(self.embed_dim, use_bias=self.use_bias,
...
class LegacyPooling(keras.Layer):
    def __init__(self, num_seeds, embed_dim, seed_dim, **kwargs):
        ...
        self.seed_vector = self.add_weight(shape=(num_seeds, seed_dim or embed_dim),
                                           initializer="glorot_uniform", trainable=True)
        self.feedforward = FFN(embed_dim=embed_dim,
                               **{key: value for key, value in kwargs.items() if key not in ('num_heads', 'layer_norm')})
    ...
    def build(self, input_shape):
        ...
        self.mab.build((input_shape[0], self.num_seeds, self.seed_dim or self.embed_dim), transformed)

    def call(self, x, training=False, **kwargs):
        transformed = self.feedforward(x, training=training)
        seeds = keras.ops.tile(keras.ops.expand_dims(self.seed_vector, 0), [keras.ops.shape(x)[0], 1, 1])
...
class DeepSet(bf.networks.DeepSet):
    def __init__(self, summary_dim=16, embed_dim=64, depth=2, mlp_widths=(64,),
                 inner_pooling='mean', num_heads=4, num_seeds=4, seed_dim=None,
                 expansion_factor=4.0, glu_variant='swiglu', use_bias=False,
                 layer_norm=True, activation='silu', kernel_initializer='he_normal',
                 dropout=0.05, **kwargs):
        SummaryNetwork.__init__(self, **kwargs)
        self.summary_dim = summary_dim
        self.equivariant_modules = [EquivariantLayer(
            embed_dim=embed_dim, mlp_widths=mlp_widths, pooling=inner_pooling,
            activation=activation, kernel_initializer=kernel_initializer,
            dropout=dropout, layer_norm=layer_norm) for _ in range(depth)]
        self.pooling_by_attention = LegacyPooling(
            num_seeds=num_seeds, embed_dim=mlp_widths[-1], seed_dim=seed_dim,
            num_heads=num_heads, dropout=dropout, expansion_factor=expansion_factor,
            glu_variant=glu_variant, kernel_initializer=kernel_initializer,
            use_bias=use_bias, layer_norm=layer_norm)
