...
def build_workflow(model, config):
    ...
    summary_network = bf.networks.DeepSet(summary_dim=config.summary_dim,base_distribution="normal")
...
def train_one(model, config, save=True):
    ...
    history = workflow.fit_online(epochs=config.epochs, 
...
def train_all():
    for model, summary_dim, num_obs in itertools.product(MODELS, SUMMARY_DIMS, NUM_OBS_VALUES):
        config = TrainingConfig(num_dims=20, num_obs=num_obs, summary_dim=summary_dim,
                                epochs = 256, batch_size = 64, num_batches = 128, learning_rate = 1e-4, seed = 2025)
