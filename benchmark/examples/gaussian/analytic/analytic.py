...
class GaussianAnalytical:
    ...
    def __init__(self, obs_data: np.ndarray, mu_prior_mean: float|np.ndarray, mu_prior_std: float, 
        ...
        if mu_prior_mean.ndim == 0:
            ...
            raise ValueError(f"mu_prior_mean must have shape ({num_dims},), got {mu_prior_mean.shape}")

        if obs_data.shape != (num_obs, num_dims):
            raise ValueError(f"obs_data must have shape ({num_obs}, {num_dims}), got {obs_data.shape}")
    ...
    def analytical_posterior(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        mu_post_mean = mu_post_var * (self.num_obs * x_bar / (self.likelihood_std ** 2) + 1 * self.mu_prior_mean / (self.mu_prior_std ** 2)) #(D,)
