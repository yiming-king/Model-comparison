...
class DirectSimulator:
    def __init__(self, num_dims: int, num_obs: int, 
                 likelihood_std: float, rng=None):
    ...
    def get_direct_simulator(self):
        def prior_0():
            mu=self.rng.normal(loc=0,scale=1,size=self.num_dims)
            return dict(mu=mu)
        def prior_10():
            mu=self.rng.normal(loc=10,scale=1,size=self.num_dims)
            return dict(mu=mu)
        def likelihood_n(mu):
            x=self.rng.normal(loc=mu,scale=self.likelihood_std,size=(self.num_obs,self.num_dims))
            return dict(x=x)
        def likelihood_t(mu):
            x=self.rng.standard_t(df=10, size=(self.num_obs,self.num_dims))*self.likelihood_std+mu
            return dict(x=x)
