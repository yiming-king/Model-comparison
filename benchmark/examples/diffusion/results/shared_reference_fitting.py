...
def _validate_legacy_direct(references, approximator, manifest, model, settings, sd):
    ...
    if set(sd.REFERENCE_METRICS) - set(references):
        return None
    fit = sd.summary_outputs(
        approximator, load_reference_partition(manifest, model, "fit")
    )
    calibration = sd.summary_outputs(
        approximator, load_reference_partition(manifest, model, "calibration")
    )
    prior = references["mmd"]["reference_summary"]
    if fit.shape != np.shape(prior) or not np.allclose(
        fit, prior, rtol=1e-6, atol=1e-7
    ):
        return None
    ...
    def check(actual, expected):
        if not np.allclose(actual, expected, rtol=2e-5, atol=2e-6):
            raise ValueError(
                "Legacy reference does not match shared-data recalculation"
            )
    ...
    try:
        for metric in ("l2", "linf"):
            ...
            check(mean, ref["mean"])
            check(chol, ref["chol"])
            scores = sd.summary_distance_from_summary(
                calibration, mean, chol, metric=metric
            )
            stats = sd.bootstrap_interval(
                scores,
        ...
        scores = sd.mmd_reference_distance_from_summary(
        ...
        for key, value in zip(
            ("low", "median", "high"), np.quantile(values, [lower, 0.5, 1 - lower])
        ):
            check(value, ref[key])
    except (ValueError, KeyError, TypeError):
        return None
    validation = sd.summary_outputs(
        approximator, load_reference_partition(manifest, model, "validation")
    )
    references["density"].update(
        sd.density_flow_validation(
            references["density"]["flow"],
            validation,
    ...
    return {
        ...
        "verification_rtol": 2e-5,
        "verification_atol": 2e-6,
        "validation": "recomputed using persisted shared validation observations",
    }


def load_or_fit_shared_reference(
    approximator,
    *,
    checkpoint,
    model,
    ...
    if (
        ...
        and previous_identity.get("simulator_sha256")
        == manifest["generation"]["simulator_sha256"]
    ):
        references = sd.load_references(path)
        migration = _validate_legacy_direct(
            references, approximator, manifest, model, settings, sd
        )
    if migration is None:
        print(
            f"Fitting {owner}/{model} against the persisted shared raw reference bank",
            flush=True,
        )
        keras.utils.set_random_seed(identity["effective_seed"])
        references = sd.fit_reference_suite(
            approximator,
            SharedReferenceSimulator(manifest, model),
            metrics=metrics,
            **{**settings, "seed": identity["effective_seed"]},
        )
    else:
        print(
            f"Verified {owner}/{model} legacy fit/calibration; refreshed shared validation",
            flush=True,
        )
