"""Diffusion diagnostics in each direct classifier's own learned summary space.
...
def _checkpoint_config(path, loss):
    ...
    score = config["config"]["inference_network"]["config"]["scoring_rules"]["scoring_rule"]["class_name"]
    expected = {"cross_entropy": "CrossEntropyScore", "exponential": "ExponentialScore", "logistic": "LogisticScore"}
...
def _load_observations(indirect_path, num_obs):
    ...
    if index[keys].isna().any().any() or index.duplicated(keys).any() or index.empty:
        raise ValueError("Indirect observations must have unique nonempty dataset/ID keys")
    arrays = {}
    ...
    y = np.stack([arrays[key] for key in index[keys].itertuples(index=False, name=None)])
...
def diagnostic_frame(index, distances, references, loss):
    """Normalize candidate-specific scores; low surprise is not high surprise."""
    frames = []
    for metric in DIAGNOSTICS:
        all_high = np.logical_and.reduce([
            np.asarray(distances[m][metric]) > references[m][metric]["high"] for m in MODELS
        ])
        for model in MODELS:
            ref = references[model][metric]
            distance = np.asarray(distances[model][metric], dtype=float)
            low, median, high = (float(ref[k]) for k in ("low", "median", "high"))
            if not np.isfinite([low, median, high]).all() or not low <= median < high:
                raise ValueError(f"Invalid reference for {loss}/{model}/{metric}")
            if distance.shape != (len(index),) or not np.isfinite(distance).all():
                raise ValueError(f"Invalid diagnostic scores for {loss}/{model}/{metric}")
            frame = index[[c for c in ("dataset", "id", "generating_model") if c in index]].copy()
            frame["loss_function"], frame["diagnostic"], frame["assumed_model"] = loss, metric, model
            frame["summary_owner"], frame["summary_dim"] = f"direct_{loss}", ref["summary_dim"]
            frame["d_M"] = distance
...
def _comparison_frame(index, approximator, observations, indirect_tag):
    ...
    direct = np.asarray(approximator.estimate(conditions=data_conditions(observations))["model_probs"], dtype=float)
...
def _score_summaries(summary, references, sd):
    ...
    for model in MODELS:
        distances[model] = {}
        for metric, ref in references[model].items():
            if metric == "mmd":
                distance = sd.mmd_reference_distance_from_summary(summary, ref["reference_summary"], ref["bandwidth2"], ref["reference_kernel_mean"])
            elif metric == "density":
                distance = -sd.signed_typicality_from_summary(summary, ref["flow"], ref["expected_log_density"])
            else:
                distance = sd.summary_distance_from_summary(summary, ref["mean"], ref["chol"], metric=metric)
            distances[model][metric] = distance
    return distances


def ensure_direct_diagnostics(
    loss, *, results_dir=RESULT_DIR / "direct", indirect_tag="S4D",
    checkpoint=None, shared_reference_root=None,
    overwrite=False, **settings,
):
    ...
    metric-calibration or observed datasets. Four density flows are fitted per classifier.
    """
    if loss not in LOSSES:
        raise ValueError(f"Unknown direct loss: {loss}")
    if set(settings) - DEFAULT_SETTINGS.keys():
        raise ValueError(f"Unknown reference settings: {set(settings) - DEFAULT_SETTINGS.keys()}")
    ...
    indirect_path = RESULT_DIR / "NPE_results" / f"npe_{indirect_tag}_all_observed_results_gold.csv"
    observations, index = _load_observations(indirect_path, num_obs)
    from ..results.shared_reference_data import ensure_shared_reference_data, reference_data_identity
    from ..results.shared_reference_fitting import load_or_fit_shared_reference
    shared_manifest = ensure_shared_reference_data(
        root=shared_reference_root,
        **{key: settings[key] for key in ("n_fit", "n_calibration", "n_density_validation", "seed")},
    )
    ...
    reference_identity = {
        "schema_version": 2, "summary_owner": f"direct_{loss}",
        "shared_reference_data": reference_data_identity(shared_manifest),
        "shared_fitting_implementation_sha256": file_sha256(BASE_DIR / "results/shared_reference_fitting.py"),
        "checkpoint": str(checkpoint), "checkpoint_sha256": file_sha256(checkpoint),
        "summary_dim": summary_dim, "num_obs": num_obs, "model_order": list(MODELS),
        "settings": settings,
        "implementation_sha256": file_sha256(Path(__file__)),
        "reference_implementation_sha256": file_sha256(BASE_DIR / "results/summary_diagnostic.py"),
        ...
        "observation_loader_sha256": file_sha256(BASE_DIR / "results/observed_datasets.py"),
        "conditions_sha256": _array_sha256(observations[0, :, 1]),
    }
    identity = {
        **reference_identity, "indirect_configuration": indirect_tag,
        "indirect_path": str(indirect_path), "indirect_sha256": file_sha256(indirect_path),
    ...
    if not overwrite and metadata_path.exists() and all(p.exists() for p in outputs):
        ...
        if metadata.get("identity") == identity and metadata.get("output_sha256") == {str(p.relative_to(output)): file_sha256(p) for p in outputs}:
    ...
    for model in MODELS:
        references[model] = load_or_fit_shared_reference(
            approximator, checkpoint=checkpoint, model=model, owner=f"direct_{loss}",
            path=diagnostics / f"reference_{model}.pkl", manifest=shared_manifest,
            metrics=DIAGNOSTICS, settings=settings, overwrite=overwrite,
            allow_legacy_direct=True,
        )
    frame = diagnostic_frame(index, _score_summaries(summary, references, sd), references, loss)
    _write_csv(csv_path, frame)
    np.savez_compressed(summary_path, summary=summary, dataset=index["dataset"].to_numpy(str), id=index["id"].to_numpy(str))
    created = datetime.now(timezone.utc).isoformat()
    _write_json(output / "pmp_comparison.metadata.json", {
        "created_at_utc": created, "loss_function": loss, "num_datasets": len(index),
        "direct_checkpoint": str(checkpoint), "checkpoint_sha256": identity["checkpoint_sha256"],
        "direct_summary_dimension": summary_dim, "model_order": list(MODELS),
        "model_prior": [0.25] * 4, "indirect_configuration": indirect_tag,
        "indirect_source": str(indirect_path), "gold_source": "shared Stan bridge-sampling PMP in the indirect observed cache",
    ...
    _write_json(metadata_path, {
        ...
        "output_sha256": {str(p.relative_to(output)): file_sha256(p) for p in outputs},
        ...
        "classification": "all high surprise iff every candidate distance exceeds its own upper reference bound",
        ...
        "density_validation": {m: {k: v for k, v in references[m]["density"].items() if k.startswith("density_validation_")} for m in MODELS},
    })
...
if __name__ == "__main__":
    ...
    for loss in args.loss or LOSSES:
        ensure_direct_diagnostics(loss, indirect_tag=args.indirect_tag, overwrite=args.overwrite)
