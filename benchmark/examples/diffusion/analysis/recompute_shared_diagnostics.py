...
def configurations():
    ...
    return ["S4D", "S1D", "S2D", "S6D"] + [
        ...
        for suffix in ("noMMD", "noMMD_rerun1")
        for multiplier in (1, 2, 4, 6)
    ]
...
def run_indirect(tag):
    ...
    if not result_file.exists():
        raise FileNotFoundError(
            f"This command requires existing inference results: {result_file}"
        )
    keras.utils.disable_interactive_logging()
    approximators = load_approximators(config)
    references = load_or_fit_reference_suite(
        approximators, config, metrics=REFERENCE_METRICS
    )
    results = pd.read_csv(result_file, keep_default_na=False)
    diagnostics = ensure_observed_summary_diagnostics(
        config, results, approximators, references
    )
    print(
        f"Completed {tag}: {len(results)} observed datasets, {len(diagnostics)} diagnostics",
        flush=True,
    )
...
def main():
    ...
    group.add_argument(
        "--direct-loss", choices=("cross_entropy", "exponential", "logistic")
    )
    ...
    if args.indirect_tag:
        ...
        for name, flags in tasks:
            ...
            with (logs / f"{name}.log").open("a") as stream:
                result = subprocess.run(
                    [
                        sys.executable,
                        "-m",
                        __package__ + ".recompute_shared_diagnostics",
                        *flags,
                    ],
                    stdout=stream,
            ...
            if result.returncode:
                write_json(
                    logs / "status.json",
                    {**status, "failed": name, "exit_code": result.returncode},
                )
